...
def index(request):
    if request.method == 'GET':
        ...
        try:
            ...
            query = Q(name__contains=s_id) | Q(ra=s_id)
            students = Student.objects.filter(query).all()
            results = {
                'students': students
                }
...
def discipline(request, code, year, semester, classes):
    try:
        if classes:
            ...
            # How its the most especif query, it renders the page of the discipline
            return render(request, 'stalkeador/discipline.html', {'discipline':discipline})
        elif semester:
